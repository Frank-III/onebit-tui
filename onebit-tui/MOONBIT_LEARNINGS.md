...
// ❌ This doesn't work
struct Container {
  item : Renderable  // Error: Cannot use trait as type
}
```
...
struct CustomRenderable {
  render_fn : (Buffer, UInt, UInt, UInt, UInt) -> Unit
}
```

3. **Potential Future Pattern (unconfirmed):**
```moonbit
// May work with references (similar to Rust's dyn)
struct Container {
  items : Array[&Renderable]  // Needs verification
}
```
...
```json
{
  "import": [
    "moonbit-community/onebit-tui/core",
    "moonbit-community/onebit-tui/ffi"
  ]
}
```
...
*This document is a living reference. Update it as new patterns and solutions are discovered.*
