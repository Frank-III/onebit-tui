# Agent Guidelines for opentui (mostly sync upstream for opentui and work one onebit-tui)

take a look at moon-llm.txt and maybe scan over the moon-agent.txt and @onebit-tui/ROADMAP.md . I want you to scan over onebit-tui and plan about what would be our next move, always try to come up with a good api that is moonbit native before write the code. Remember the moon command is your best friend
